...
VertexShader = 
{
	...
	[[
		VS_OUTPUT_BORDER main( const VS_INPUT_BORDER VertexIn )
		{
			...
			VertexOut.uv = sqrt(VertexIn.uv) * 1.1;
...
PixelShader = 
{
	...
	[[
		float4 main( VS_OUTPUT_BORDER Input ) : PDX_COLOR
		{
			...
			float vGlowFactor;
			if (Input.uv[0] > 0.48f) {
				float uvModified = Input.uv[0] / 1.1f - 0.35f;
				vGlowFactor = 0.135f / uvModified;
			}
			else {
				float uvModified = Input.uv[0] * 1.1f + 0.15f;
				vGlowFactor = min(uvModified, 0.88f);
				if (vGlowFactor == 0.88f) {
					vGlowFactor = 0.0f;
				}	
			}
